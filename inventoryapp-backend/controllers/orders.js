...
const getAllOrders = async (req, res) => {
  ...
  const filteredPreviousPeriodOrders = await orders.filter(
    order => new Date(order.createdAt) >= previousPeriod
  );

  const filteredPreviousPreviousPeriodOrders = await orders.filter(order => {
    let orderDate = new Date(order.createdAt);
    return orderDate <= previousPeriod && orderDate >= previousPreviousPeriod;
  });

  const currentPeriodNumberOfOrders = await filteredPreviousPeriodOrders.length;

  const currentPeriodNumberOfProducts =
    await filteredPreviousPeriodOrders.reduce(
      (count, order) => (count += order.products.length),
      0
    );

  const currentPeriodRevenue = await filteredPreviousPeriodOrders.reduce(
    (totalRevenue, order) => (totalRevenue += order.grossTotal),
    0
  );

  const previousPeriodNumberOfOrders =
    await filteredPreviousPreviousPeriodOrders.length;

  const previousPeriodNumberOfProducts =
    await filteredPreviousPreviousPeriodOrders.reduce(
      (count, order) => (count += order.products.length),
      0
    );
